...
def calc_perc_err(
    ref: np.ndarray, x: np.ndarray, ord: int = 2, auto_normalize: bool = True
):
    ...
    with dev:
        if auto_normalize:
            ...
            err = xp.linalg.norm((ref - x).ravel(), ord=ord)  # type: ignore
...
def cg(
    ...
    A: sp.linop.Linop,
    b: np.ndarray,
    P: Optional[sp.linop.Linop] = None,
    ...
    def calculate_objective(x):
        ...
        if lamda:
            obj += sp.to_device(lamda * xp.linalg.norm(x) ** 2, sp.cpu_device)
        return obj
    ...
    with device:
            ...
            if pAp > 0:
                ...
                if draw_output:
                    try:
                        ...
                        image_slice = x  # 2D
                    image_slice_cpu = np.rot90(abs(sp.to_device(image_slice, -1)), 3)
                    ...
                    if k < 144:
                        ...
                        ax.set_title("Iter %d" % (k + 1), fontsize="x-small", y=0.92)
        ...
        if draw_output:
            ...
            ax1.set_title(r"$\frac{1}{2} ||Ax - b||^2_2 + \lambda||x||^2_2 $ using CG")
...
def admm(
    ...
    A: sp.linop.Linop,
    b: np.ndarray,
    lst_proxg: List[Callable],
    rho: float,
    lst_g: Optional[List[Callable]] = None,
    method: str = "cg",
    P: Optional[sp.linop.Linop] = None,
    ref: Optional[np.ndarray] = None,
    save: Optional[str] = None,
