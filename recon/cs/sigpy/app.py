...
class App(object):
    ...
    def run(self):
        ...
        if self.show_pbar:
            ...
            self.pbar = tqdm(total=self.alg.max_iter, desc=name, leave=self.leave_pbar)
...
class LinearLeastSquares(App):
    ...
    def _get_alg(self):
        ...
        if self.solver == "ConjugateGradient":
            ...
            raise ValueError("Invalid solver: {solver}.".format(solver=self.solver))
    ...
    def _get_PrimalDualHybridGradient(self):
        ...
        if self.lamda > 0:
            ...
            proxg = prox.L2Reg(self.x.shape, self.lamda, y=self.z, proxh=self.proxg)
        else:
            gamma_primal = 0
            if self.proxg is None:
            ...
            if self.lamda > 0:
                if self.z is None:
                    ...
                    obj += (
                        self.lamda
                        / 2
                        * self.x_device.xp.linalg.norm(self.x - self.z).item() ** 2
                    )
    ...
    def __init__(
        ...
        if G is None:
            self.max_eig_app = MaxEig(
                A.N, dtype=self.x.dtype, device=self.x_device, show_pbar=show_pbar
            )
        ...
        if tau is None or sigma is None:
            max_eig = MaxEig(
                A.H * A, dtype=self.x.dtype, device=self.x_device, show_pbar=show_pbar
            ).run()
            tau = 1
            sigma = 1 / max_eig
        ...
        alg = PrimalDualHybridGradient(
            proxfc, proxg, A, A.H, self.x, self.u, tau, sigma, max_iter=max_iter
        )
